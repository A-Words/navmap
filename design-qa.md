...
- Route panel: route mode uses shadcn ToggleGroup, route points use compact input rows, and the active route point renders a shared `RouteFieldSuggestions` component directly beneath the input. The component shows the Apple Maps-style "我的位置" row for an empty field and the route search results list for a typed query, so both surfaces share the same placement and route-card treatment.
...
- Browser check: passed for the shared route input suggestion component. Empty active route input renders "我的位置" in `.route-field-suggestions`; typing `广州` swaps the same container to route search result rows; no route search results render outside the route card.
