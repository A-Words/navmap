...
- Route panel: route mode uses shadcn ToggleGroup, route points use compact input rows, and the active route point renders a shared floating `RouteFieldSuggestions` card anchored directly beneath the input. The component shows the Apple Maps-style "我的位置" row for an empty field and the route search results list for a typed query, so both surfaces share the same placement, card shadow, rounded surface, and overlay behavior.
