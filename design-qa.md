...
- C:\Users\A_Words\AppData\Local\Temp\codex-clipboard-a351f680-3ca5-460d-bd4a-4700feb3a3ee.png
...
state: default Chinese route planning screen, compact left icon rail, independently open/collapsible route/search panels, OSM map canvas, active route overlay, top-right map quick controls, bottom-right zoom controls, bottom OSM attribution/status.
...
- Route panel: route mode uses shadcn ToggleGroup, route points use compact input rows, and the focused route point renders a shared floating `RouteFieldSuggestions` card flush with the input row. The component shows the Apple Maps-style "我的位置" row for an empty field and the route search results list for a typed query, so both surfaces share the same placement, card shadow, rounded surface, click-away dismissal, and overlay behavior.
...
- Map controls: layer and current-location buttons now sit as compact floating controls at the map's top-right edge, while zoom in/out controls sit at bottom-right; OSM attribution remains visible in the bottom status surface.
...
- Browser check: passed at 1440 x 1024 for the route planning reference. Focusing the origin field shows "我的位置" directly below the active input inside the route card, top-right map controls expose two buttons for layer/current location, bottom-right zoom controls expose two buttons, and the old map toolbar/layer switcher selectors are absent.
- Browser check: passed for the shared route input suggestion component. Empty active route input renders "我的位置" in `.route-field-suggestions`; typing `广州` swaps the same container to route search result rows; no route search results render outside the route card. The latest CSS pass makes `.route-field-suggestions` an absolutely positioned floating card above the form content instead of an in-flow list, and the interaction pass hides it when clicking outside the route input/suggestion area. A 1440 x 1024 Browser pass confirmed the route suggestion card opens on input press, sits flush with the input row (`gap: 0`), closes on map click, and reopens when pressing the same input again.
